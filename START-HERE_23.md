...
**Priority 1 — Clean up test data:**
Go to Supabase → Table Editor → impersonation_reports and platform_verifications. Delete all rows. Then delete your account entirely and sign up fresh as a real paying member to experience the full journey.

**Priority 2 — Full member journey test:**
Pay with a real card. Go through: signup → email confirm → payment → ID verification → link platforms → log an impersonation report → generate a case file. Experience it exactly as a member would before any promotion.

**Priority 3 — Landing page updates:**
- Update step 04 copy: honest framing — guiding members to report, not promising removal
- Update impersonation case file description in comparison table (now a per-report formal document)
- Review £0 blue tick stat wording

**Priority 4 — Member welcome email:**
Email sent after government ID verification is confirmed. "Your REAL ID is confirmed. Here's your profile link." Not yet built.

---

## FUTURE BUILDS — AGREED

- Member welcome email (post ID verification confirmed)

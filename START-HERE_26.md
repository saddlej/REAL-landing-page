...
30. **Case file is generated on the fly — not stored.** A future session will build case file logging (case_files table in Supabase) so case references become real verifiable records.
...
**Case file:** Formal per-report document for legal escalation. Opens in new browser tab via blob URL. Contains:
...
182. Hero search input — transparent on mobile via !important CSS ✅
183. Hero search bar — unified pill shape (input and button as one seamless unit, overflow:hidden on wrapper) ✅
184. Join the founding waitlist button — rounded pill styling ✅
185. Dashboard — back button on mobile no longer navigates to login page (popstate handler added to all modals) ✅
186. All Reports modal — back button from report pack returns to All Reports, not dashboard ✅
187. Case file — renders full width on mobile via blob URL approach (replaces document.write) ✅
188. Login page — show/hide password toggle added (eye icon inside password field) ✅
189. members table — full_name column confirmed exists in Supabase ✅
190. Stripe webhook (api/stripe-webhook.js) — saves verified full_name to members table on gov ID confirmation ✅
191. Dashboard — full_name now fetched alongside other member data ✅
192. Case file — case reference number added (REAL-[first 8 chars of report ID, uppercase]) ✅
193. Case file — legal name field added (shows full_name or privacy-safe fallback if blank) ✅
194. Case file — evidence reference line added to incident section ✅
195. Case file — Action Requested section added after Registry Statement ✅
...
**Priority 1 — Case file logging:**
Build a `case_files` table in Supabase to log every case file generated. Columns: member ID, report ID, case reference (REAL-XXXXXXXX), generated_at timestamp. This makes case references real and verifiable — a solicitor or platform can ask "did REAL issue this?" and we can confirm it. Without this, the case reference is cosmetic.
...
- Case file logging — case_files table in Supabase (Priority 1 next session)
- Member welcome email (post ID verification confirmed) — Priority 2 next session

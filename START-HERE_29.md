...
# Version 29 — Updated 20 June 2026
...
These principles came directly from tonight's landing page work and should guide all future copy decisions:
...
219. Landing page — new scarcity line added directly above "Founding Members — 100 spots only" section: "Every day you wait, someone else takes a lower number. RL-000001 is gone the moment someone else claims it." ✅
...
**Priority 1 — Waitlist / founding member numbering honesty gap (discovered 20 June 2026):**

A real gap was found in how the waitlist relates to the 100 founding member spots. Currently:
- The waitlist form says "be first in line for a founding member spot" — implying an order is being tracked.
- In reality, no order or position is recorded anywhere when someone joins the waitlist. The waitlist table just stores name, email, handle, platform_request with no position/rank field.
- REAL ID numbers are ONLY assigned sequentially at the moment Stripe webhook fires confirming payment — not at waitlist signup.
- This means the true "race" for low founding numbers happens at the point of payment when REAL opens, not at the point of joining the waitlist. Whoever pays first gets the lower number, regardless of waitlist join order.
- If more than 100 people join the waitlist (likely, since there's no cap on waitlist size), some will join expecting the £15 founding rate and a low number, and will not get either.

**This needs solving before any more founding-member/scarcity copy is added to the page.** Sadi flagged this directly — questioning whether the messaging was fair/honest given the mechanic doesn't currently support what's implied. This is exactly the kind of honesty gap that breaks the "REAL never overclaims" principle if left unaddressed.

**Options to consider next session (none decided yet):**
- Add an actual position/rank field to the waitlist table and email people their real position
- Be explicit that waitlist join order does NOT guarantee a founding spot — payment order does — and reword copy accordingly
- Send a clear, honest email to anyone who doesn't make the first 100 explaining they'll be a standard member at £25/month, no hard feelings, same product
- Some combination of the above

Sadi wants to tackle this first thing next session, before continuing any other landing page work.

**Priority 2 — Member welcome email:**
...
**Priority 3 — Remaining landing page work (Hormozi framework pass, in progress 20 June 2026):**
- "Mechanism" section — why REAL works when nothing else does. Discussed at length tonight. Agreed direction: short, confident, 3-4 lines, NOT a process explanation (that's what "How REAL works" already does). Should answer "why should I believe this works" without over-explaining, similar in spirit to how Instagram/TikTok never explained their mechanics upfront, just the outcome. Last agreed draft: "Every other form of verification is owned by someone else — a platform, a company, an algorithm. They can take it away the moment it suits them. REAL is owned by you. Independent. Permanent. Yours." NOT YET BUILT — still needs final sign-off and placement decided (likely between problem section/founder quote and "How REAL works").
- The gap between "Founding Members — 100 spots only" section and the waitlist form needs content. Multiple copy directions explored tonight (see below) but none finalised — Sadi felt early drafts sounded "beggy" or not authentic to his voice. Last direction explored (not finalised): plain, honest explanation of why the founding discount exists — it's a £15 vs £25 thank-you for early belief, not a "better" product — written in Sadi's direct, "I give a shit," non-needy voice. This ties directly into Priority 1 above and should likely be solved together.
- Social proof section — still doesn't exist anywhere on the page. Cannot use fake/invented testimonials (against REAL's honesty principles). Wait until real founding members exist, then add genuine credited quotes.

**Priority 4 — Remaining builds in order:**

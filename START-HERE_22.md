...
**⚠ KNOWN ISSUE — next session priority:**
The impersonation_reports table does not store which scenario was selected. When "Get report pack →" is clicked on an existing report card, it cannot read back the original scenario and defaults to Scenario 1 text. Fix requires:
1. Run in Supabase SQL Editor: ALTER TABLE impersonation_reports ADD COLUMN situation text;
2. Update dashboard.html to save situation value on report submission
3. Update openReportPack to read situation from stored report and use correct scenario key

**Report card on dashboard:**
- Platform name, fake handle in red, date noticed
- "Get report pack →" button — retrieves pre-written text and platform link

**Public profile:**
- Shows "X impersonation attempts documented by REAL" — live count from impersonation_reports table
...
id, user_id, platform, fake_handle, verified_handle, date_noticed, notes, created_at, situation (⚠ situation column NOT YET ADDED — next session priority)
...
**Priority 1 — Fix situation not saving to database:**
Run in Supabase SQL Editor:
ALTER TABLE impersonation_reports ADD COLUMN situation text;
Then update dashboard.html to save situation on submit and read it back in openReportPack.

**Priority 2 — Waitlist confirmation email via Resend:**
Olivia has the link. If she signs up to the waitlist she needs an automated confirmation email. Simple "you're on the list, we'll be in touch" email via Resend. Build before any promotion.

**Priority 3 — Clean up test data:**
Clear all test reports and test verifications from Supabase before going live as a paying member.

**Priority 4 — Full member journey test:**
Pay with a real card, go through the entire flow as a member, experience it properly before content creation or promotion.

**Priority 5 — Landing page updates:**
- Update step 04 copy: honest framing — guiding members to report, not promising removal
- Update impersonation case file description in comparison table
- Review £0 blue tick stat wording
...
- Member statement on lost accounts — timestamped public declaration
...
**Olivia (elevatewitholivia)** — Runs Elevate Digital Academy. 50+ fake TikTok accounts. Completed her course on launch day. Has seen the REAL link. Replied warmly. Instagram DM sent — ball in her court. Do not chase further until she responds.
...
**Natalie Ellis, Bossbabe** — Same approach.

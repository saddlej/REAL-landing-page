...
# Version 31 — Updated 27 June 2026
...
3. **REAL ID numbers are only assigned when government ID verification is confirmed.** Payment comes first (the members table row is created at payment), but the REAL ID itself is only generated and written when the `identity.verification_session.verified` webhook fires. Fixed 27 June 2026 — was previously being assigned at payment, which was wrong.
...
13. **The confirmed member journey order is:** Sign up → Email confirm → Pay (members table row created here, no REAL ID yet) → Government ID verification completes → REAL ID assigned + welcome email fires at this exact moment → Platform linking unlocks (greyed out and disabled in dashboard until `id_verified` is true). Corrected 27 June 2026 — was listed incorrectly as "Link platforms → REAL ID assigned."
...
229. BUG FIX — REAL ID was being assigned at payment (`checkout.session.completed`) instead of at government ID verification. Moved REAL ID generation logic from payment handler into `identity.verification_session.verified` handler in `api/stripe-webhook.js`. Payment still creates the member row (without a REAL ID). REAL ID only assigned when ID verification actually clears. Fixed 27 June 2026. ✅
230. BUG FIX — Hero search was not filtering out unverified members (those with null real_id, i.e. paid but not yet ID verified). Added `real_id=not.is.null` filter to both idUrl and nameUrl queries in index.html at the Supabase query level. Fixed 27 June 2026. ✅
231. Member welcome email built and live — fires from `identity.verification_session.verified` handler in `api/stripe-webhook.js` immediately after REAL ID is assigned. Sent via Resend. Matches waitlist email design exactly: navy header, white shield (shield-email-white.png). Confirmed landing in primary inbox (not spam). Copy matches approved version exactly. ✅
232. Welcome email test trigger built — hidden GET branch inside `api/stripe-webhook.js`, gated by `TEST_EMAIL_KEY` environment variable. Does not count as a separate serverless function. Stays in place for future debugging. URL format: `/api/stripe-webhook?key=SECRET&name=NAME&realid=RL-XXXXXX&email=address`. ✅
233. `TEST_EMAIL_KEY` environment variable added to Vercel. ✅
234. BUG FIX — "Go to my dashboard" button in welcome email was producing a 404. Added `/dashboard` rewrite rule to `vercel.json` (same pattern as existing `/RL-:id` rule). Email button already pointed to the clean `/dashboard` URL — the rule just needed to exist. Fixed 27 June 2026. ✅
235. Confirmed: email confirmation link (from Supabase's built-in email after signup) takes members straight to the dashboard already logged in — no interstitial "email confirmed" page. The `email-confirmed.html` page is for confirming an email *as a linked platform*, not for the initial signup confirmation — these are two separate flows. ✅
236. Confirmed: `login.html`, `dns-guide.html`, and `email-confirmed.html` still link to `/dashboard.html` rather than the new clean `/dashboard` address. Not broken (Vercel serves `.html` files directly), purely cosmetic — flagged for future tidy-up only. ✅
...
**Priority 1 — Confirm two open questions before building anything else:**

1. **Does `api/create-verification-session.js` enforce payment before ID verification can begin?** As the code stands, the "Start verification →" button on the dashboard calls this file directly without any visible payment check on the dashboard side. We need Claude Code to read this file and confirm whether it checks for an existing paid member row before issuing a Stripe Identity session. If it doesn't, someone could theoretically skip payment and go straight to ID verification — a real integrity gap.

2. **Is the "Founding Member — £15.00/month" text on the dashboard live data or hardcoded?** The subscription section in `dashboard.html` shows this text but it may be a hardcoded placeholder that shows identically for every member regardless of their actual tier. Needs confirming — a standard £25 member should not see "Founding Member" if they aren't one.

**Priority 2 — Full Stripe test-mode end-to-end signup:**
A complete fake signup in Stripe's test mode (fake card, fake ID documents, sandbox environment) to prove the entire member journey fires correctly in sequence: signup → confirm email → pay → ID verify → REAL ID assigned → welcome email fires → platform linking unlocks. Deliberately deferred until the two open questions above are answered.
...
- Hormozi-style launch sequence (mapped 20 June 2026, deliberately parked)
- `/dashboard.html` → `/dashboard` link tidy-up across login.html, dns-guide.html, email-confirmed.html (cosmetic only, not broken)
...
| 14 | Onboarding / welcome email | ✅ Complete — built 27 June 2026, fires on ID verification, confirmed live |
...
- Confirm `api/create-verification-session.js` enforces payment before ID verification (open question — top priority next session)
- Confirm subscription section on dashboard shows real member data, not hardcoded placeholder (open question)
- Full Stripe test-mode end-to-end signup
- `/dashboard.html` → `/dashboard` link tidy-up across login.html, dns-guide.html, email-confirmed.html (cosmetic only)
...
- Assigned only when `identity.verification_session.verified` webhook fires (government ID confirmed) — NOT at payment. Fixed 27 June 2026.
